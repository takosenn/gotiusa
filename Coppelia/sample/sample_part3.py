# sample_part3.py
from sample_part2 import *

# ro_i（targetと各Agentの距離）表示用テキスト（グラフ外右側に配置）     →いらんと思う
"""roi_text = ax.text(
    0.05,
    0.10,
    "",
...
def animate(i):
    ...
    for j in range(num_agents):
        ...
        # VisionSensorで距離が測定できた場合のvec計算
        if ro_i is not None:
            # VisionSensorの向きから角度を取得
            vision_sensor_orientation = sim.getObjectOrientation(
                visionSensorHandles[j], -1
            )
            # VisionSensorの向きを考慮してtargetの方向ベクトルを計算
            # VisionSensorの向き（yaw角）を取得
            yaw_angle = vision_sensor_orientation[2]  # Z軸回転（yaw）
            # VisionSensorの向き方向の単位ベクトル
            vision_direction = np.array([np.cos(yaw_angle), np.sin(yaw_angle)])
            # targetの方向ベクトル（VisionSensorの向き方向に距離ro_i）
            vec = vision_direction * ro_i
            print(
                f"[Drone{j+1}] VisionSensor値からvec計算: ro_i = {ro_i:.2f} m, 角度 = {np.degrees(yaw_angle):.1f}°"
            )
        else:
            # 計算値でvecを求める
